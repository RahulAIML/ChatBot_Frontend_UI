...
const baseQuery = fetchBaseQuery({
  ...
  baseUrl: `${import.meta.env.VITE_FC_API_URL}`,
  /* @ts-ignore */
  ...
  prepareHeaders: (headers) => {
    ...
    const fcForgotToken = localStorage.getItem('fcForgotToken');
    
    
    if (mcafcToken) {
      headers.set('authorization', `Bearer ${mcafcToken}`);
    }
    if (fcForgotToken) {
      headers.set('authorization', `Bearer ${fcForgotToken}`);
    }
...
export const apiSliceFC = createApi({
  reducerPath: 'api3',
  baseQuery: baseQueryWithReauth,
  // tagTypes: ["DriverList"],
  endpoints: () => ({}),
