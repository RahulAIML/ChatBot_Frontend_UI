...
import { typeDetailForBValidate } from 'src/validation/candidate/typeDetails';
import Title2 from 'src/components/common/title2';

import {
  useUpdateTypeDetailsBMutation,
...
const Type = ({ data, updateRedux }) => {
  ...
  const validate = async (_e) => {
    try {
      _e.preventDefault();
      typeDetailForBValidate.parse(state);
    } catch (error) {
      console.log(error);
  ...
  return (
    <Box component="main" mt={2}>
      <Title2 title="Your Type of Candidature :" title2=" Maharashtra State Candidature Type B" />
          ...
          <Grid container spacing={2}>
            <Grid item xs={12} md={6}>
              <Typography variant="subtitle2" display={'inline-flex'}>
                Whose domicile Cerificate You are uploading at {import.meta.env.VITE_FC_NAME} ?{' '}
                <Typography
                  variant="subtitle2"
                  display={'inline-flex'}
                  sx={{ color: 'error.main' }}
            ...
            <Grid item xs={12} md={6}>
              ...
              >
                <FormControlLabel value="C" control={<Radio size="small" />} label="Candidate" />
            ...
            <Grid item xs={12} md={6}>
              <Typography variant="subtitle2" display={'inline-flex'}>
                Select the District from where Candidate / Father / Mother of Candidate is Domiciled
